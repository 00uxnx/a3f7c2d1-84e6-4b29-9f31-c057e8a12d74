...
local BUILTINS = {
	["print"]=true, ["warn"]=true, ["error"]=true, ["tostring"]=true,
	["tonumber"]=true, ["type"]=true, ["pairs"]=true, ["ipairs"]=true,
	["next"]=true, ["select"]=true, ["unpack"]=true, ["rawget"]=true,
	["rawset"]=true, ["setmetatable"]=true, ["getmetatable"]=true,
	["pcall"]=true, ["xpcall"]=true, ["assert"]=true, ["loadstring"]=true,
	["require"]=true, ["collectgarbage"]=true, ["rawequal"]=true,
	["wait"]=true, ["delay"]=true, ["spawn"]=true, ["tick"]=true,
	["time"]=true, ["elapsedTime"]=true, ["typeof"]=true,
	["Instance"]=true, ["Vector3"]=true, ["Vector2"]=true,
	["CFrame"]=true, ["Color3"]=true, ["BrickColor"]=true,
	["UDim"]=true, ["UDim2"]=true, ["Rect"]=true, ["Ray"]=true,
	["Region3"]=true, ["NumberSequence"]=true, ["ColorSequence"]=true,
	["NumberRange"]=true, ["TweenInfo"]=true, ["Enum"]=true,
	["math"]=true, ["string"]=true, ["table"]=true, ["os"]=true,
	["task"]=true, ["coroutine"]=true, ["bit32"]=true, ["utf8"]=true,
	["printidentity"]=true,
}
local GLOBALS = {
	["game"]=true, ["workspace"]=true, ["script"]=true, ["plugin"]=true,
	["_G"]=true, ["shared"]=true, ["_VERSION"]=true,
}
...
local EXECUTOR_GLOBALS = {
	...
	["newcclosure"]=true, ["islclosure"]=true, ["iscclosure"]=true,
	["checkcaller"]=true, ["getcallingscript"]=true,
	["getscripts"]=true, ["getloadedmodules"]=true,
	["getinstances"]=true, ["getnilinstances"]=true,
	["getconnections"]=true, ["fireclickdetector"]=true,
	["firetouchinterest"]=true, ["fireproximityprompt"]=true,
	["cloneref"]=true, ["compareinstances"]=true,
	["readfile"]=true, ["writefile"]=true, ["appendfile"]=true,
	["listfiles"]=true, ["makefolder"]=true, ["delfile"]=true,
	["delfolder"]=true, ["isfile"]=true, ["isfolder"]=true,
	["loadfile"]=true,
	["request"]=true, ["http_request"]=true, ["HttpGet"]=true,
	["getcustomasset"]=true,
	["getclipboard"]=true, ["setclipboard"]=true, ["toclipboard"]=true,
	["identifyexecutor"]=true, ["getexecutorname"]=true,
	["messagebox"]=true, ["queue_on_teleport"]=true,
	["decompile"]=true, ["getscriptbytecode"]=true,
	["getscripthash"]=true, ["getscriptfunction"]=true,
	["Drawing"]=true, ["cleardrawcache"]=true, ["getrenderproperty"]=true,
	["setrenderproperty"]=true, ["isrenderobj"]=true,
	["debug"]=true, ["base64encode"]=true, ["base64decode"]=true,
	["crypt"]=true, ["bit"]=true,
	["cache"]=true, ["saveinstance"]=true,
}
...
-- library functions 
-- executor functions
function library:CheckIfExecutorCanDo(func_name)
	local tests = {
		-- file system
		["readfile"] = function() return type(readfile) == "function" end,
		["writefile"] = function() return type(writefile) == "function" end,
		["appendfile"] = function() return type(appendfile) == "function" end,
		["listfiles"] = function() return type(listfiles) == "function" end,
		["makefolder"] = function() return type(makefolder) == "function" end,
		["delfile"] = function() return type(delfile) == "function" end,
		["delfolder"] = function() return type(delfolder) == "function" end,
		["isfile"] = function() return type(isfile) == "function" end,
		["isfolder"] = function() return type(isfolder) == "function" end,
		["loadfile"] = function() return type(loadfile) == "function" end,
		...
		["toclipboard"] = function()
			return type(toclipboard) == "function"
		end,
		...
		["setrawmetatable"] = function()
			return type(setrawmetatable) == "function"
		end,
		...
		["getcallingscript"] = function()
			return type(getcallingscript) == "function"
		end,

		["getscripts"] = function()
			return type(getscripts) == "function"
		end,

		["getloadedmodules"] = function()
			return type(getloadedmodules) == "function"
		end,

		-- instances
		["getinstances"] = function()
			return type(getinstances) == "function"
		end,

		["getnilinstances"] = function()
			return type(getnilinstances) == "function"
		end,

		["cloneref"] = function()
			return type(cloneref) == "function"
		end,

		["compareinstances"] = function()
			return type(compareinstances) == "function"
		end,
		...
		["http_request"] = function()
			return type(http_request) == "function"
				or type(request) == "function"
		end,

		["HttpGet"] = function()
			return typeof(game.HttpGet) == "function"
		end,
		...
		["setrenderproperty"] = function()
			return type(setrenderproperty) == "function"
		end,

		["isrenderobj"] = function()
			return type(isrenderobj) == "function"
		end,
		...
		["getexecutorname"] = function()
			return type(getexecutorname) == "function"
				or type(identifyexecutor) == "function"
		end,
		...
		["getscripthash"] = function()
			return type(getscripthash) == "function"
		end,

		["getscriptfunction"] = function()
			return type(getscriptfunction) == "function"
		end,

		-- crypt / encoding
		["crypt"] = function()
			return type(crypt) == "table"
		end,

		["base64encode"] = function()
			return type(base64encode) == "function"
				or (type(crypt) == "table"
				and type(crypt.base64encode) == "function")
		end,

		["base64decode"] = function()
			return type(base64decode) == "function"
				or (type(crypt) == "table"
				and type(crypt.base64decode) == "function")
		end,
	...
	return result and "supported" or "unsupported"
end

function library:executorinit()
	local supported = {}
	local unsupported = {}

	local folder = library.directory .. "/supportedandunsupported"
	local filepath = folder .. "/results.txt"

	if not isfolder(folder) then
		makefolder(folder)
	end

	for globalName in pairs(EXECUTOR_GLOBALS) do
		local status = self:CheckIfExecutorCanDo(globalName)
...
function library:colorize(text, color_hex)
	return string.format('<font color="#%s">%s</font>', color_hex, text)
end

function library:tokenize(src)
	local result = {}
	local i = 1
	local len = #src
	...
	while i <= len do
		...
		if c == "\n" then
			result[#result+1] = "\n"
			i = i + 1
		elseif src:sub(i, i+1) == "--" and src:sub(i, i+3) ~= "--[[" then
			local j = src:find("\n", i, true) or (len + 1)
			result[#result+1] = library:colorize(library:escape_rich(src:sub(i, j-1)), TOKEN_COLORS.comment)
			i = j
		elseif src:sub(i, i+3) == "--[[" then
			local j = src:find("]]", i+4, true) or len
			result[#result+1] = library:colorize(library:escape_rich(src:sub(i, j+1)), TOKEN_COLORS.comment)
			i = j + 2
		elseif c == '"' then
			local j = i + 1
			while j <= len do
				...
				elseif src:sub(j,j) == '"' then j = j + 1; break
				else j = j + 1 end
			end
			result[#result+1] = library:colorize(library:escape_rich(src:sub(i, j-1)), TOKEN_COLORS.string)
			i = j
		elseif c == "'" then
			local j = i + 1
			while j <= len do
		...
		elseif src:sub(i, i+1) == "[[" then
			local j = src:find("]]", i+2, true) or len
			result[#result+1] = library:colorize(library:escape_rich(src:sub(i, j+1)), TOKEN_COLORS.string)
			i = j + 2
		elseif c:match("%d") or (c == "." and src:sub(i+1,i+1):match("%d")) then
			...
			if src:sub(i, i+1):lower() == "0x" then
				...
				while j <= len and src:sub(j,j):match("[%d%.eExX_]") do j = j + 1 end
			end
			result[#result+1] = library:colorize(library:escape_rich(src:sub(i, j-1)), TOKEN_COLORS.number)
			i = j
		elseif c:match("[%a_]") then
			local j = i
			while j <= len and src:sub(j,j):match("[%w_]") do j = j + 1 end
			local word = src:sub(i, j-1)
			local escaped = library:escape_rich(word)
			if KEYWORDS[word] then
				result[#result+1] = library:colorize(escaped, TOKEN_COLORS.keyword)
			elseif LITERALS[word] then
				result[#result+1] = library:colorize(escaped, TOKEN_COLORS.literal)
			elseif BUILTINS[word] then
				result[#result+1] = library:colorize(escaped, TOKEN_COLORS.builtin)
			elseif GLOBALS[word] then
				result[#result+1] = library:colorize(escaped, TOKEN_COLORS.global)
			elseif EXECUTOR_GLOBALS[word] then
				result[#result+1] = library:colorize(escaped, TOKEN_COLORS.executor)
			else
				result[#result+1] = library:colorize(escaped, TOKEN_COLORS.default)
			end
			i = j
		elseif c:match("[%+%-%*/%%^#&|~<>=%(%)%{%}%[%]%;:,%.%!]") then
			result[#result+1] = library:colorize(library:escape_rich(c), TOKEN_COLORS.operator)
			i = i + 1
		elseif c == " " or c == "\t" then
			result[#result+1] = c
			i = i + 1
		else
			result[#result+1] = library:escape_rich(c)
			i = i + 1
		end
	end
...
function library:window(properties)
	...
	local main_window = library:panel({
		name = properties and properties.name or "Comet.wtf | ", 
	...
	local watermark = library:watermark({default = os.date('Comet.wtf |  - %b %d %Y - %H:%M:%S')})  

	task.spawn(function()
		while task.wait(1) do 
			watermark.change_text(os.date('Comet.wtf - Beta - %b %d %Y - %H:%M:%S'))
		end 
	end) 
	...
	-- highlight label (behind textbox, renders colored richtext)
	...
	highlight_label.Size = dim2(1, -6, 1, -6)
	highlight_label.Position = dim2(0, 3, 0, 3)
	...
	-- actual textbox (transparent text, on top for input)
	...
	-- wire tokenizer to highlight label
